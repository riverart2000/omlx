...
def _get_submodule(root, path: str):
    """Navigate an MLX module tree by dotted path (numeric parts index into
    list-like children, e.g. `blocks.0.attn.proj`). Raises AttributeError/
    IndexError/KeyError if any segment is missing."""
    m = root
    for p in path.split("."):
        if p.isdigit():
            m = m[int(p)]
...
def merge_lora_into_model(
    ...
    Handles BOTH plain `nn.Linear` and quantized `nn.QuantizedLinear` targets.
    For a quantized target the packed weight can't be added to directly (its
    stored shape is the bit-packed layout, e.g. (out, in/4) for 8-bit), so we
    dequantize -> add the LoRA delta -> re-quantize with the module's own
    bits / group_size / mode. This is what makes `cfg_step_lora` /
    `refinement_lora` usable against the q4 / q8 variants.

    Args:
        mx_model: an nn.Module whose submodules include the LoRA target Linears.
        lora_state_dict: the loaded safetensors dict from the LoRA file.
        ...
        target_prefix: optional path prefix prepended to each decoded module_path
            before lookup. Use `""` for a flat model; use `"dit"` if your wrapper
            nests the DiT under `self.dit`.
    ...
    for module_path, group in grouped.items():
        full_path = f"{target_prefix}.{module_path}" if target_prefix else module_path
        try:
            mod = _get_submodule(mx_model, full_path)
        ...
        delta = compute_merged_delta(group, multiplier=multiplier)

        if hasattr(mod, "scales"):
            # Quantized target: dequantize -> merge -> re-quantize in place.
            bits = mod.bits
            group_size = mod.group_size
            mode = getattr(mod, "mode", "affine")
            base_w = mx.dequantize(
                mod.weight, mod.scales,
                getattr(mod, "biases", None),
                group_size=group_size, bits=bits, mode=mode,
            )
            if tuple(delta.shape) != tuple(base_w.shape):
                raise ValueError(
                    f"shape mismatch merging {module_path}: "
                    f"delta {tuple(delta.shape)} vs dequantized base {tuple(base_w.shape)}"
                )
            merged = base_w + delta.astype(base_w.dtype)
            ...
            base_w = mod.weight
            delta = delta.astype(base_w.dtype)
            if tuple(delta.shape) != tuple(base_w.shape):
                raise ValueError(
                    f"shape mismatch merging {module_path}: "
                    f"delta {tuple(delta.shape)} vs base {tuple(base_w.shape)}"
                )
            mod.weight = base_w + delta
